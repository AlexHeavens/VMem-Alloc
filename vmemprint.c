...
void print_block(unsigned long* block){
	...
	printf("pos abs(%lu) header \t- value %lu - %s - size(words) %lu\n",
		(unsigned long)block, (unsigned long)*block, isUsed, get_size(block)/sizeof(void*));

	// Print it depending on the type.
	if(get_used(block))print_used_block(block);
	else print_free_block(block);
}
...
void print_free_block(unsigned long* block){
	...
	printf("previous ");
	if(!get_prev_free(block))printf("(none) - ");
	else printf("(%lu) - ", (unsigned long)get_prev_free(block));
	printf("next ");
	if(!get_next_free(block))printf("(none)\n");
