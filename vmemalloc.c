...
// Before any trace data is written the collumn headers are written. Set to 0 when trace data is output.
...
void outputTraceColumnHeaders(){
	...
	fprintf(traceFile,"Function\tTime(us)\tAlloc Mem\tFree Mem\tNo. Alloc\tNo. Free\tLargest Free\n");
}
...
void outputTraceData(const char *op){
	...
	if(traceFile!=NULL){
		...
		//This implementation is dumb so it ignores some of the required trace data.
		fprintf(traceFile,"%s\t%lu\t%lu\t%lu\t%lu\t%lu\t%lu\n",op,execution_time(),
			currentAllocatedMemory,totalFreeSpace,currentAllocations, currentFreeRegions, largestFreeRegion);
	}
}
...
void *vmemalloc(int size){
	...
	if(!last_mmap){
		...
		debug_printf("Using bit masks used_bm(%lu) and size_bm(%lu)", used_bm, size_bm);
		debug_printf("Header size is %lu.", HEADER_SIZE);
	}
	
	// If there is not currently enough free space, mmap more.
	if(largestFreeRegion < (unsigned long)size + HEADER_SIZE + MIN_BLOCK_SIZE){
...
/**
Adds a block to the free list, updating the statistics if it is going from used to free.  This works on an address-
ordered basis.
...
void free_add(unsigned long* block){
	...
	if(block){
		...
		// If we are adding a used block to the free list, set it as free and update the statistics.
		if(get_used(block)){
			set_used(block, FREE);
			...
			if(get_size(block) > largestFreeRegion) largestFreeRegion = get_size(block);
		}
		...
		if(free_start == NULL){
			...
			free_start = block;
			free_end = block;
			debug_printf("Added %lu to the start of the free list.", (unsigned long) block);
			return;
		}

		// If the new free block is before the start of the free list, make it the start.
		if(block < free_start){
		...
		while(current != NULL){
			...
			// If the next node has a greater address, insert the block into the free list here.
			if(block < current){
...
void header_stamp(unsigned long* start, unsigned long size, unsigned long used){
	set_size(start, size);
	set_used(start, used);
	debug_printf("Block %lu now has size %lu.", (unsigned long)start, get_size(start));
}

unsigned long get_used(unsigned long* block){
	return ((*block) & used_bm);
}

unsigned long get_size(unsigned long* block){
	return ((*block) & size_bm);
}
...
unsigned long* get_next_free(unsigned long* block){
	...
	return *((unsigned long**)((void*)block + HEADER_SIZE + sizeof(unsigned long*)));
}

void set_prev_free(unsigned long* block, unsigned long* newPrev){
	assert(block != NULL);
	*((unsigned long**)((void*)block + HEADER_SIZE)) = newPrev;
	debug_printf("Setting prev free of %lu to %lu.", (unsigned long)block, (unsigned long)newPrev);
}

void set_next_free(unsigned long* block, unsigned long* newNext){
	assert(block != NULL);
	*((unsigned long**)((void*)block + HEADER_SIZE + sizeof(unsigned long*))) = newNext;
	debug_printf("Setting next free of %lu to %lu.", (unsigned long)block, (unsigned long)newNext);
}

/**
Finds a block in the free list that is large enough to store the requested size.  Splits the free block if it is larger.  
Works on a "first-fit" basis.
...
void* find_free_block(unsigned long size){
	...
	unsigned long min = (size >= MIN_BLOCK_SIZE - HEADER_SIZE)? size : MIN_BLOCK_SIZE - HEADER_SIZE;
	...
	while(current != NULL){
		...
		if(get_size(current) >= min){
			...
			// If there is not enough spare space to create a new free block, take it all.
			if((get_size(current) - min) < MIN_BLOCK_SIZE){
				...
				if(current == free_start){
					...
					set_next_free(get_prev_free(current), get_next_free(current));
					set_prev_free(get_next_free(current), get_prev_free(current));
				}
				
				// If we have taken (possibly) the largest free region, we need to update the field.
				...
				unsigned long* newBlock = (asc_addrs)?
					((void*)current + min + HEADER_SIZE) : ((void*)current + get_size(current) - min);
				unsigned long oldSize = get_size(current);
				unsigned long newSize = (oldSize - min - HEADER_SIZE);
				
				// Depending on the architecture, it may be cause less fragmentation to stamp from the 
				// bottom or top on a block.
				if(asc_addrs){
			...
			debug_printf("Returning new allocated block %lu, size %lu.", (unsigned long)current, get_size(current));
...
unsigned long largest_free_region(){
	...
	while(current != NULL){
		...
		if(get_size(current) > currentLargest)currentLargest = get_size(current);
		current = get_next_free(current);
	}
...
/**
Coalesces a block forwards.  Works on the assumption that the next free block is free.
*/
void coalesce_forward(unsigned long* block){
...
/**
Coalesces a block backwards.  Works on the assumption that the previous free block is free.
*/
void coalesce_backward(unsigned long* block){
	assert(!get_used(block));
...
void coalesce(unsigned long* firstBlock, unsigned long* secondBlock){
	...
	debug_printf("(Coalescing %lu and %lu)", (unsigned long) firstBlock, (unsigned long) secondBlock);
	
	// Remove the second block from the free list.
	set_next_free(firstBlock, get_next_free(secondBlock));
	if(secondBlock != free_end)set_prev_free(get_next_free(secondBlock), firstBlock);
	else free_end = firstBlock;
	
	// Adjust the size of the first block.
	unsigned long newSize = get_size(secondBlock) + get_size(firstBlock) + HEADER_SIZE;
	set_size(firstBlock, newSize);
	
	// Update the stats.
	totalFreeSpace += HEADER_SIZE;
	currentFreeRegions--;
	if(get_size(firstBlock) > largestFreeRegion) largestFreeRegion = get_size(firstBlock);
}

/**
Given a block, this will check if the previous and latter blocks can be coalesced, which it will do.
*/
void coalesce_both(unsigned long* block){
	...
	if(prevBlock){
		...
		if(!get_used(prevBlock) && ((void*)prevBlock + get_size(prevBlock) + HEADER_SIZE)  == block)
			coalesce_backward(block);
	}
	
	// If the next block isn't the end of the memory space and it is free, coalesce it.
	unsigned long* nextBlock = get_next_free(block);
	if(nextBlock){
		if(!get_used(nextBlock) && ((void*)block + get_size(block) + HEADER_SIZE)  == nextBlock)
...
unsigned long* allocate_mem(unsigned long size){
	...
	debug_printf("Have mmapped size %lu at location %lu.", numPages * sysconf(_SC_PAGESIZE), (unsigned long) newBlock);
	asc_addrs = (newBlock > last_mmap);
	last_mmap = newBlock;
	
	// Stamp it as free and add to the free list.
	header_stamp(newBlock, numPages * sysconf(_SC_PAGESIZE) - HEADER_SIZE, FREE);
	
	// Update stats.
	currentFreeRegions++;
	totalFreeSpace += get_size(newBlock);
	if(get_size(newBlock) > largestFreeRegion)largestFreeRegion = get_size(newBlock);
